...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int x = 0; x < board.size(); x++) {
            for (int y = 0; y < board.size(); y++) {
                Tile tile = tile(x, y);
                if(x-1>0&&tile(x-1,y).value()==tile.value()){return true;}
                if(x+1<board.size()&&tile(x+1,y).value()==tile.value()){return true;}
                if(y-1>0&&tile(x, y-1).value()==tile.value()){return true;}
                if(y+1<board.size()&&tile(x, y+1).value()==tile.value()){return true;}
            }
        }
        return false;
    }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        for(targetY=y+1;targetY < board.size(); targetY++) {
            Tile tile = board.tile(x, targetY);
            if(tile==null){
                continue;
            }
            if(tile.value()==myValue){
                if (!tile.wasMerged() && !currTile.wasMerged()) {
                    targetY++;
    ...
    public void tilt(Side side) {
        ...
        board.setViewingPerspective(side);//set the view perspective with "side"
        for (int x = 0; x < board.size(); x++) {
            tiltColumn(x);
        }
        board.setViewingPerspective(Side.NORTH);//reset the view in north
